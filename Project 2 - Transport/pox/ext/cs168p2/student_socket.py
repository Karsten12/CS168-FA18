...
class StudentUSocket(StudentUSocketBase):
  ...
  def connect(self, ip, port):
    ...
    myPacket = self.new_packet(ack=False, data=None, syn=True)


    myPacket.tcp.seq = self.snd.una # set seq num to ISN
  ...
  def tx(self, p, retxed=False):
    ...
    # if (retxed is False and (p.tcp.SYN or p.tcp.FIN or p.tcp.payload)):
    # 	p.tx_ts = self.stack.now
    # 	# self.log.error(str(p.tcp.seq) + "HI")
    # 	self.retx_queue.push(p)

    if (p.tcp.SYN or p.tcp.FIN or p.tcp.payload) and not retxed:
      # p.retxed = retxed
      p.tx_ts = self.stack.now # Complete for Stage 8	
      self.retx_queue.push(p) # Complete for Stage 8	
      # pass
  ...
  def rx(self, p):
    ...
    * Send pending ACKs and FINs
    ...
    if self.state is CLOSED:
      return
    # Complete for Stage 1
    # If src host still in SYN_SENT mode, and src receives pkt from remote, most likey SYN_ACK then handle it
    elif self.state is SYN_SENT:
    ...
    while (not self.rx_queue.empty()):
        newP = self.rx_queue.peek()
        newPseq = newP[0]
        if (newPseq |GT| self.rcv.nxt):
        	# packet is out of order
        	self.set_pending_ack()
        	break
        else:
        	newP = self.rx_queue.pop()[1]
        	newSeg = newP.tcp
        	newPayload = newP.app
        	data = newPayload[self.rcv.nxt |MINUS| newP.tcp.seq:]
        	self.handle_accepted_seg(newSeg, data)
  ...
  def handle_synsent (self, seg):
    ...
    if acceptable_ack:
      # Complete for Stage 1
      # next expected seq num for the server is basically the 
      # sequence num of the pckt the server sent + 1
      self.rcv.nxt = seg.seq |PLUS| 1
      # For the client, the oldest unacked seq num, is the one from the packet
      # that it just recieved from the server, bc it hasn't responded to the server
  ...
  def update_rto(self, acked_pkt):
    ...
    # self.log.debug(str(self.rttvar))
    # pass
  ...
  def update_window(self, seg):
    ...
    self.snd.wnd = seg.win #self.TX_DATA_MAX # remove w/ implemented
  ...
  def handle_accepted_ack(self, seg):
    ...
    # Complete Stage 8
    # self.snd.una is a sequence number
    acked_pkts = self.retx_queue.pop_upto(seg.ack)
  ...
  def check_ack(self, seg):
    ...
    if (self.state == LAST_ACK):
    	if (self.fin_ctrl.acks_our_fin(ack = seg.ack)):
    		self._delete_tcb()
  ...
  def maybe_send(self):
    ...
    while remaining > 0:
      ...
      wnd_space_left = snd.wnd |MINUS| bytes_sent

      # wnd_space_left = snd.wnd |MINUS| snd.una |MINUS| snd.nxt # ALT FOR STAGE 8

      # payload_size = min(remaining, self.mss)

      payload_size = min(remaining, self.mss, wnd_space_left)
      ...
      # remaining_bytes = snd.wnd |MINUS| (bytes_in_flight |PLUS| bytes_sent)

      # payload_size = min(remaining, self.mss, remaining_bytes)
  ...
  def check_timer_retx(self):
    ...
    if (not self.retx_queue.empty()):
    	seq_and_packet = self.retx_queue.get_earliest_pkt() 
    	p = seq_and_packet[1]
